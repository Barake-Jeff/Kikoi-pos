...
const getPrinterPath = () => {
    if (app.isPackaged) {
      // In production (installed app)
      return join(process.resourcesPath, 'bin', 'printer_engine.exe');
    }
    // In development
    return join(__dirname, '..', 'resources', 'bin', 'printer_engine.exe');
};
...
function startBackend() {
  try {
    ...
    if (backendStartAttempts >= MAX_BACKEND_ATTEMPTS) {
      console.error(
        `[Electron Main] Max backend start attempts (${MAX_BACKEND_ATTEMPTS}) reached. Quitting.`,
      );
      dialog.showErrorBox(
        "Backend Startup Failed",
        `The backend server could not be started after ${MAX_BACKEND_ATTEMPTS} attempts. Please check logs. The application will now close.`,
      );
      app.quit();
      return;
    }
    ...
    console.log(
      `[Electron Main] Attempting to start backend (Attempt ${backendStartAttempts}/${MAX_BACKEND_ATTEMPTS}).`,
    );
    ...
    if (!fs.existsSync(backendPath)) {
      throw new Error(
        `Backend entry point not found at: ${backendPath}. Make sure the backend is built.`,
      );
    }
    ...
    dialog.showErrorBox(
      ...
      `A fatal error occurred while trying to start the backend: ${error.message}`,
    );
    app.quit();
  }
}
...
function pingBackend() {
  ...
  const performPing = () => {
    ...
    if (pingAttempts >= maxPings) {
      console.error(
        "[Electron Main] Backend did not respond within the timeout period.",
      );
      dialog.showErrorBox(
        "Backend Timeout",
        `The backend server did not respond within ${BACKEND_PING_TIMEOUT / 1000} seconds.`,
      );
...
function createWindow() {
  ...
  mainWindow.loadURL(frontendUrl).catch((err) => {
    ...
    dialog.showErrorBox(
      "Frontend Load Error",
      `Could not load the application UI. Please check if the frontend is running or built correctly. Error: ${err.message}`,
    );
  });
...
app.on("ready", () => {
  ...
  // Register IPC handlers early so renderer can call them before backend is ready
  ipcMain.handle("get-app-version", () => app.getVersion());

  ipcMain.handle("print-receipt", async (event, receiptData) => {
    return new Promise((resolve, reject) => {
        const printerPath = getPrinterPath();
        console.log("[Electron Main] Spawning printer service at:", printerPath);

        if (!fs.existsSync(printerPath)) {
            console.error("[Electron Main] Printer executable not found at:", printerPath);
            return resolve({ success: false, error: "Printer service executable missing" });
        }

        const pyProcess = spawn(printerPath);
        ...
        pyProcess.on('close', (code) => {
            console.log(`[Electron Main] Printer process exited with code ${code}`);
            if (code !== 0) {
                console.error('[Electron Main] Python Error:', errorData);
                resolve({ success: false, error: errorData || 'Printer process failed' });
            } else {
                try {
                    resolve(JSON.parse(result));
                } catch (e) {
                    console.log("[Electron Main] Raw result:", result);
  ...
  ipcMain.handle("preview-receipt", async (event, receiptData) => {
      // NOTE: Python service doesn't support visual preview yet. 
      // We could keep the old logic if needed, but for now we focus on the new solution.
      return { success: false, error: "Preview not supported in Python mode" };
  });

  ipcMain.handle("list-printers", async () => {
    console.log("[Electron Main] list-printers handler invoked");
    ...
    try {
      ...
      if (typeof (webContents as any).getPrintersAsync === "function") {
        console.log("[Electron Main] Using getPrintersAsync()");
        printers = await (webContents as any).getPrintersAsync();
      } else if (typeof (webContents as any).getPrinters === "function") {
        console.log("[Electron Main] Using getPrinters()");
        printers = (webContents as any).getPrinters();
      }

      console.log("[Electron Main] Printers found:", printers?.length || 0);
      return printers || [];
    } catch (err: any) {
      console.error("[Electron Main] Error listing printers:", err.message);
      return [];
    }
  });

  // Log that handlers were registered
  console.log(
    "[Electron Main] IPC handlers registered: get-app-version, print-receipt, list-printers",
  );
  ...
  backendStatus.on("ready", () => {
    ...
    // Handlers were already registered at app ready; nothing to do here.
  });
});
...
app.on("before-quit", () => {
  ...
  if (backendProcess && !backendProcess.killed) {
    ...
    console.log(
      `[Electron Main] Backend process termination signal sent. Success: ${killed}`,
    );
  }
});
